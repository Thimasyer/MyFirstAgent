...
const LOCAL_FAST_DOWNWARD_DRIVER = join(
    LOCAL_FAST_DOWNWARD_ROOT,
    'fast-downward.py'
);
...
const USE_DOCKER_PAAS =
    process.env.PDDL_USE_DOCKER === 'true' ||
    process.env.PDDL_USE_DOCKER === '1';
const DOCKER_PAAS_BASE_URL =
    process.env.PDDL_DOCKER_URL || 'http://localhost:5001';
const DOCKER_PAAS_SOLVER = process.env.PDDL_DOCKER_SOLVER || 'lama-first';
const DOCKER_PAAS_REQUEST_TIMEOUT_MS = Number(
    process.env.PDDL_DOCKER_REQUEST_TIMEOUT_MS || 30000
);
const DOCKER_PAAS_POLL_INTERVAL_MS = Number(
    process.env.PDDL_DOCKER_POLL_INTERVAL_MS || 500
);
const DOCKER_PAAS_MAX_POLL_ATTEMPTS = Number(
    process.env.PDDL_DOCKER_MAX_POLL_ATTEMPTS || 60
);
...
export async function solveOnline(domain, problem) {
    ...
    try {
        ...
        console.warn(
            '[PDDL] Local planner returned no plan, falling back to remote solver.'
        );
    } catch (error) {
        console.warn(
            '[PDDL] Local planner unavailable or failed:',
            error.message
        );
...
export function buildProblem(state) {
    ...
    const tileLookup = new Map(
        tiles.map((tile, index) => [`${tile.x},${tile.y}`, tileNames[index]])
    );
    ...
    lines.push(
        `    (at ${agent.id} ${tileLookup.get(`${agent.x},${agent.y}`)})`
    );
    ...
    if (objective) {
        ...
        if (objective.type === 'pickup') {
            ...
            if (objective.parcelId) {
                lines.push(
                    `    (has-parcel ${agent.id} ${objective.parcelId})`
                );
            }
        ...
        if (goalTile) {
            lines.push(
                `    (at ${agent.id} ${tileLookup.get(`${goalTile.x},${goalTile.y}`)})`
            );
        }
...
async function solveRemote(domain, problem) {
    ...
    console.log(
        `[PDDL] Requesting plan from solver (timeout: ${DEFAULT_TIMEOUT_S}s)...`
    );
    ...
    console.log(
        `[PDDL] Planner response received (${(elapsedMs / 1000).toFixed(2)}s)`
    );
...
async function solveDockerPaaS(domain, problem) {
    ...
    if (!initResponse.ok) {
        throw new Error(
            `Docker PaaS request failed ${initResponse.status} ${initResponse.statusText}`
        );
    }
    ...
    while (attempts < DOCKER_PAAS_MAX_POLL_ATTEMPTS) {
        ...
        if (!resultResponse.ok) {
            throw new Error(
                `Docker PaaS result request failed ${resultResponse.status} ${resultResponse.statusText}`
            );
        }
        ...
        if (resultJson.status && resultJson.status !== 'PENDING') {
            ...
            if (
                ...
                throw new Error(
                    `Docker PaaS planning failed: ${JSON.stringify(resultJson)}`
                );
            }
        }
...
async function solveLocal(domain, problem) {
    ...
    const domainFile = join(
        tmpdir(),
        `pddl_domain_${Date.now()}_${Math.random().toString(36).slice(2)}.pddl`
    );
    const problemFile = join(
        tmpdir(),
        `pddl_problem_${Date.now()}_${Math.random().toString(36).slice(2)}.pddl`
    );
    const planFile = join(
        tmpdir(),
        `pddl_plan_${Date.now()}_${Math.random().toString(36).slice(2)}.txt`
    );
    const sasFile = join(
        tmpdir(),
        `pddl_sas_${Date.now()}_${Math.random().toString(36).slice(2)}.sas`
    );
    ...
    const args = [
        ...
        '--build',
        LOCAL_FAST_DOWNWARD_BUILD,
        '--alias',
        LOCAL_FAST_DOWNWARD_ALIAS,
        '--overall-time-limit',
        String(LOCAL_PLANNER_OVERALL_TIME_LIMIT_S),
        '--plan-file',
        planFile,
        '--sas-file',
        sasFile,
    ...
    try {
        await execFileAsync('python3', args, {
            timeout: LOCAL_PLANNER_TIMEOUT_MS
        });
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            ...
            if (code === 0) {
                ...
                reject(
                    new Error(
                        `Local planner failed with exit code ${code}: ${stderr.trim() || stdout.trim()}`
                    )
                );
            }
        });
        ...
        if (options.timeout) {
            timeoutId = setTimeout(() => {
                ...
                reject(
                    new Error(
                        `Local planner timed out after ${options.timeout}ms`
                    )
                );
            }, options.timeout);
        }
...
async function initializeSession() {
    ...
    if (message.error) {
        throw new Error(
            `MCP initialize error: ${JSON.stringify(message.error)}`
        );
    }
...
async function sendMcpRequest(jsonRpcMessage, sessionId, retryCount = 0) {
    try {
        ...
        if (process.env.DEBUG_PDDL) {
            console.debug(
                '[PDDL] MCP raw response text:',
                JSON.stringify(text)
            );
            console.debug(
                '[PDDL] MCP parsed SSE messages:',
                JSON.stringify(sseMessages, null, 2)
            );
        }

        if (!response.ok) {
            const errorPayload =
                sseMessages.find((msg) => msg.json)?.json ?? text;
            throw new Error(
        ...
        const finalMessage =
            sseMessages.find((msg) => msg.json && msg.json.jsonrpc === '2.0') ??
        ...
        if (retryCount < MAX_RETRIES) {
            ...
            console.warn(
                `[PDDL] Request failed (attempt ${retryCount + 1}/${MAX_RETRIES + 1}), retrying in ${waitMs}ms...`
            );
