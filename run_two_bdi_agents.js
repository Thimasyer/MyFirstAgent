...
function parseArgs(argv) {
    ...
    for (let i = 2; i < argv.length; i += 1) {
        ...
        if (arg.startsWith('--token1=')) {
            ...
            args.nbrFailedAction1 = arg
                .slice('--nbrFailedAction1='.length)
                .trim();
        } else if (arg.startsWith('--nbrFailedAction2=')) {
            args.nbrFailedAction2 = arg
                .slice('--nbrFailedAction2='.length)
                .trim();
...
function printUsage() {
    console.log(
        'Usage: node run_two_bdi_agents.js --token1=<token1> --token2=<token2> [--host=<host>]'
    );
    console.log('');
    console.log(
        'You may also set TOKEN1, TOKEN2, and HOST in .env or environment.'
    );
    console.log('');
    console.log('Options:');
    ...
    console.log(
        '  --host=<url>            Game server URL (default: http://localhost:8080)'
    );
    console.log(
        '  --nbrFailedAction1=<n> Max failed actions before giving up for agent 1 (default: 2)'
    );
    console.log(
        '  --nbrFailedAction2=<n> Max failed actions before giving up for agent 2 (default: 3)'
    );
...
function main() {
    ...
    console.log(
        `[DEBUG] MAX_NUMBER_OF_FAILED_ACTION_1 exists: ${!!process.env.MAX_NUMBER_OF_FAILED_ACTION_1}`
    );
    console.log(
        `[DEBUG] MAX_NUMBER_OF_FAILED_ACTION_2 exists: ${!!process.env.MAX_NUMBER_OF_FAILED_ACTION_2}`
    );
    if (process.env.TOKEN1) {
        console.log(
            `[DEBUG] TOKEN1 first 20 chars: ${process.env.TOKEN1.slice(0, 20)}`
        );
    }
    if (process.env.TOKEN2) {
        console.log(
            `[DEBUG] TOKEN2 first 20 chars: ${process.env.TOKEN2.slice(0, 20)}`
        );
    }
    ...
    if (
        ...
        !args.token1 ||
        !args.token2 ||
        !args.nbrFailedAction1 ||
        !args.nbrFailedAction2
    ) {
        printUsage();
